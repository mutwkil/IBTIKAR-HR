...
class payrollreportexcelwiz(models.TransientModel):
    ...
    def get_item_data(self):
        ...
        if self.from_date and self.date_end:
            ...
            worksheet.write(row, 2, 'Bank', cell_text_format)
            worksheet.write(row, 3, 'Account Number', cell_text_format)
            ...
            if payslip_ids:
                for payslip in payslip_ids:
                    ...
                    column = column + 1
                    worksheet.write(row, column, payslip.employee_id.bank_account_id.bank_id.name or '', cell_text_format_new)
                    column = column + 1
                    worksheet.write(row, column, payslip.employee_id.bank_account_id.acc_number or '', cell_text_format_new)
                    for vals in res:
                        column = column + 1
                        check = False
